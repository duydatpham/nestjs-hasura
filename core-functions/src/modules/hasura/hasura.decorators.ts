...
import { HasuraActionHandlerConfig } from './hasura.interfaces';
import {
...
export const HasuraActionHandler = (config: HasuraActionHandlerConfig) => (
  target,
  key,
  descriptor
) => {
  SetMetadata(HASURA_ACTION_HANDLER, config)(
    target,
    key,
    descriptor
  );
